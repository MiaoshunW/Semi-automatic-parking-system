...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(testbench_Object_det_M) == (NULL)) &&
         !rtmGetStopRequested(testbench_Object_det_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(testbench_Object_det_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_Object_det_M, true);
      }

      if (rtmGetStopRequested(testbench_Object_det_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testbench_Object_det_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_Object_det_M, true);
      }
    }

    testbench_Object_det_output();
    ...
    {                                  /* Sample time: [0.01s, 0.0s] */
      rtExtModeUpload(0, testbench_Object_det_M->Timing.taskTime0);
    }

    testbench_Object_det_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_Object_det_M, 0);
  ...
  testbench_Object_det_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(testbench_Object_det_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(testbench_Object_det_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(testbench_Object_det_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(testbench_Object_det_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(testbench_Object_det_M));
  }
  ...
  /* Terminate model */
  testbench_Object_det_terminate();
  terminateBackButton();
  return 0;
}
