...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("testbench_str1", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(testbench_str1_M) == (NULL)) && !rtmGetStopRequested
         (testbench_str1_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(testbench_str1_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_str1_M, true);
      }

      if (rtmGetStopRequested(testbench_str1_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testbench_str1_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_str1_M, true);
      }
    }

    testbench_str1_output();
    ...
    {                                  /* Sample time: [1.0s, 0.0s] */
      rtExtModeUpload(0, testbench_str1_M->Timing.taskTime0);
    }

    testbench_str1_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_str1_M, 0);
  ...
  testbench_str1_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(testbench_str1_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(testbench_str1_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(testbench_str1_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(testbench_str1_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(testbench_str1_M));
  }
