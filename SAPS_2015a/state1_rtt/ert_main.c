...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(state1_M) == (NULL)) && !rtmGetStopRequested
         (state1_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(state1_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(state1_M, true);
      }

      if (rtmGetStopRequested(state1_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(state1_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(state1_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.01s, 0.0s] */
      rtExtModeUpload(0, state1_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(state1_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(state1_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(state1_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(state1_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(state1_M));
  }
