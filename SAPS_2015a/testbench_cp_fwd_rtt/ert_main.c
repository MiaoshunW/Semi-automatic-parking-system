...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("testbench_cp_fwd", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(testbench_cp_fwd_M) == (NULL)) &&
         !rtmGetStopRequested(testbench_cp_fwd_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(testbench_cp_fwd_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_cp_fwd_M, true);
      }

      if (rtmGetStopRequested(testbench_cp_fwd_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testbench_cp_fwd_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_cp_fwd_M, true);
      }
    }

    testbench_cp_fwd_output();
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, testbench_cp_fwd_M->Timing.taskTime0);
    }

    testbench_cp_fwd_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_cp_fwd_M, 0);
  ...
  testbench_cp_fwd_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(testbench_cp_fwd_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(testbench_cp_fwd_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(testbench_cp_fwd_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(testbench_cp_fwd_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(testbench_cp_fwd_M));
  }
