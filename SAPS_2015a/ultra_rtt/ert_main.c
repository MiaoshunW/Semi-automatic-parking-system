...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("ultra", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(ultra_M) == (NULL)) && !rtmGetStopRequested(ultra_M)
         ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(ultra_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(ultra_M, true);
      }

      if (rtmGetStopRequested(ultra_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(ultra_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(ultra_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, ultra_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(ultra_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(ultra_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(ultra_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(ultra_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(ultra_M));
  }
