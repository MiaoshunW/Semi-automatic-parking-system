...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("testbench_state2", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while (rtmGetErrorStatus(testbench_state2_M) == (NULL) ) {
    /* Wait for the next timer interrupt */
    MW_sigWait(&info.sigMask);
    testbench_state2_output();

    /* Get model outputs here */
    testbench_state2_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_state2_M, 0);
  ...
  if (rtmGetErrorStatus(testbench_state2_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(testbench_state2_M));
  }

  /* Disable rt_OneStep() here */

  /* Terminate model */
  testbench_state2_terminate();
  terminateLCD();
  terminateBackButton();
  return 0;
}
