...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("ultrasonic1", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(ultrasonic1_M) == (NULL)) && !rtmGetStopRequested
         (ultrasonic1_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(ultrasonic1_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(ultrasonic1_M, true);
      }

      if (rtmGetStopRequested(ultrasonic1_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(ultrasonic1_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(ultrasonic1_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.03s, 0.0s] */
      rtExtModeUpload(0, ultrasonic1_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(ultrasonic1_M, 0);
  ...
  ultrasonic1_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(ultrasonic1_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(ultrasonic1_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(ultrasonic1_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(ultrasonic1_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(ultrasonic1_M));
  }
