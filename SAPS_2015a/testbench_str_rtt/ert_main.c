...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("testbench_str", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(testbench_str_M) == (NULL)) && !rtmGetStopRequested
         (testbench_str_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(testbench_str_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_str_M, true);
      }

      if (rtmGetStopRequested(testbench_str_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testbench_str_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_str_M, true);
      }
    }

    testbench_str_output();
    ...
    {                                  /* Sample time: [1.0s, 0.0s] */
      rtExtModeUpload(0, testbench_str_M->Timing.taskTime0);
    }

    testbench_str_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_str_M, 0);
  ...
  testbench_str_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(testbench_str_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(testbench_str_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(testbench_str_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(testbench_str_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(testbench_str_M));
  }
