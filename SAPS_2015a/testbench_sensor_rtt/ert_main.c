...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("testbench_sensor", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(testbench_sensor_M) == (NULL)) &&
         !rtmGetStopRequested(testbench_sensor_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(testbench_sensor_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_sensor_M, true);
      }

      if (rtmGetStopRequested(testbench_sensor_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testbench_sensor_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_sensor_M, true);
      }
    }

    testbench_sensor_output();
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, testbench_sensor_M->Timing.taskTime0);
    }

    testbench_sensor_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_sensor_M, 0);
  ...
  testbench_sensor_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(testbench_sensor_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(testbench_sensor_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(testbench_sensor_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(testbench_sensor_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(testbench_sensor_M));
  }
