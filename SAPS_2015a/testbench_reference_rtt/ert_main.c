...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("testbench_reference", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(testbench_reference_M) == (NULL)) &&
         !rtmGetStopRequested(testbench_reference_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(testbench_reference_M->extModeInfo, 2, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_reference_M, true);
      }

      if (rtmGetStopRequested(testbench_reference_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testbench_reference_M->extModeInfo, 2, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_reference_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.0s, 0.0s] */
      rtExtModeUpload(0, testbench_reference_M->Timing.t[0]);
    }

    {                                  /* Sample time: [0.01s, 0.0s] */
      rtExtModeUpload(1, ((testbench_reference_M->Timing.clockTick1) * 0.01));
    }

    testbench_reference_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_reference_M, 0);
  ...
  testbench_reference_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(testbench_reference_M));
  rtExtModeCheckInit(2);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(testbench_reference_M->extModeInfo, 2, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(testbench_reference_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(testbench_reference_M) != NULL) {
