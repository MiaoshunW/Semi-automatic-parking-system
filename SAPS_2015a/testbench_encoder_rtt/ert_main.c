...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(testbench_encoder_M) == (NULL)) &&
         !rtmGetStopRequested(testbench_encoder_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(testbench_encoder_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_encoder_M, true);
      }

      if (rtmGetStopRequested(testbench_encoder_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testbench_encoder_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_encoder_M, true);
      }
    }

    testbench_encoder_output();
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, testbench_encoder_M->Timing.taskTime0);
    }

    testbench_encoder_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_encoder_M, 0);
  ...
  testbench_encoder_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(testbench_encoder_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(testbench_encoder_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(testbench_encoder_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(testbench_encoder_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(testbench_encoder_M));
  }
