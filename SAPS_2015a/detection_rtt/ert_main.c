...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("detection", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(detection_M) == (NULL)) && !rtmGetStopRequested
         (detection_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(detection_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(detection_M, true);
      }

      if (rtmGetStopRequested(detection_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(detection_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(detection_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, detection_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(detection_M, 0);
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(detection_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(detection_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(detection_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(detection_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(detection_M));
  }
