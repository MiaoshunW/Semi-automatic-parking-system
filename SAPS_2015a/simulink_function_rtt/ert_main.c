...
void baseRateTask(void *arg)
{
  ...
  while ((rtmGetErrorStatus(simulink_function_M) == (NULL)) &&
         !rtmGetStopRequested(simulink_function_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(simulink_function_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(simulink_function_M, true);
      }

      if (rtmGetStopRequested(simulink_function_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(simulink_function_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(simulink_function_M, true);
      }
    }

    simulink_function_output();
    ...
    {                                  /* Sample time: [0.01s, 0.0s] */
      rtExtModeUpload(0, simulink_function_M->Timing.taskTime0);
    }

    simulink_function_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(simulink_function_M, 0);
  ...
  simulink_function_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(simulink_function_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(simulink_function_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(simulink_function_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(simulink_function_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(simulink_function_M));
  }
