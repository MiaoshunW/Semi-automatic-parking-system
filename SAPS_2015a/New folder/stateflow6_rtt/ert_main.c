...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("stateflow6", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(stateflow6_M) == (NULL)) && !rtmGetStopRequested
         (stateflow6_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(stateflow6_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(stateflow6_M, true);
      }

      if (rtmGetStopRequested(stateflow6_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(stateflow6_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(stateflow6_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.01s, 0.0s] */
      rtExtModeUpload(0, stateflow6_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(stateflow6_M, 0);
  ...
  stateflow6_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(stateflow6_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(stateflow6_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(stateflow6_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(stateflow6_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(stateflow6_M));
  }
