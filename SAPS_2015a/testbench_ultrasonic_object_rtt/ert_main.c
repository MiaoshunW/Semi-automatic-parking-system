...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("testbench_ultrasonic_object", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(testbench_ultrasonic_object_M) == (NULL)) &&
         !rtmGetStopRequested(testbench_ultrasonic_object_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(testbench_ultrasonic_object_M->extModeInfo, 1,
        &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_ultrasonic_object_M, true);
      }

      if (rtmGetStopRequested(testbench_ultrasonic_object_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testbench_ultrasonic_object_M->extModeInfo, 1,
                       &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_ultrasonic_object_M, true);
      }
    ...
    testbench_ultrasonic_object_output();
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, testbench_ultrasonic_object_M->Timing.taskTime0);
    }

    testbench_ultrasonic_object_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_ultrasonic_object_M, 0);
  ...
  testbench_ultrasonic_object_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(testbench_ultrasonic_object_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(testbench_ultrasonic_object_M->extModeInfo, 1,
      &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(testbench_ultrasonic_object_M, true);
    }
  ...
  if (rtmGetErrorStatus(testbench_ultrasonic_object_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(testbench_ultrasonic_object_M));
  }
  ...
  /* Terminate model */
  testbench_ultrasonic_object_terminate();
  terminateLCD();
  terminateBackButton();
  return 0;
}
