...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("testbench_straight", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(testbench_straight_M) == (NULL)) &&
         !rtmGetStopRequested(testbench_straight_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(testbench_straight_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_straight_M, true);
      }

      if (rtmGetStopRequested(testbench_straight_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(testbench_straight_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(testbench_straight_M, true);
      }
    }

    testbench_straight_output();
    ...
    {                                  /* Sample time: [0.5s, 0.0s] */
      rtExtModeUpload(0, testbench_straight_M->Timing.taskTime0);
    }

    testbench_straight_update();
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(testbench_straight_M, 0);
  ...
  testbench_straight_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(testbench_straight_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(testbench_straight_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(testbench_straight_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(testbench_straight_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(testbench_straight_M));
  }
