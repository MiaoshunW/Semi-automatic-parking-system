...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("model21", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while (rtmGetErrorStatus(model21_M) == (NULL) ) {
...
int main(int argc, char **argv)
{
  ...
  if (rtmGetErrorStatus(model21_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(model21_M));
  }
