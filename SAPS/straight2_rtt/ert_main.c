...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("straight2", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(straight2_M) == (NULL)) && !rtmGetStopRequested
         (straight2_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(straight2_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(straight2_M, true);
      }

      if (rtmGetStopRequested(straight2_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(straight2_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(straight2_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, straight2_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(straight2_M, 0);
  ...
  straight2_initialize();

  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(straight2_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(straight2_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(straight2_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(straight2_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(straight2_M));
  }
