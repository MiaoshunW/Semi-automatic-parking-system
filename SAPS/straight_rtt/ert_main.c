...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("straight", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(straight_M) == (NULL)) && !rtmGetStopRequested
         (straight_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(straight_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(straight_M, true);
      }

      if (rtmGetStopRequested(straight_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(straight_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(straight_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.1s, 0.0s] */
      rtExtModeUpload(0, straight_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(straight_M, 0);
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(straight_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(straight_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(straight_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(straight_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(straight_M));
  }
