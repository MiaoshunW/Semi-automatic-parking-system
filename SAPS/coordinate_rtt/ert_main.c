 ...
 * C/C++ source code generated on : Fri Nov 25 15:57:16 2016
 *
...
void baseRateTask(void *arg)
{
  ...
  lcdDisplayText("coordinate", 1, 20);
  initBackButton();
  MW_setTaskPeriod(info.period, info.signo);
  while ((rtmGetErrorStatus(coordinate_M) == (NULL)) && !rtmGetStopRequested
         (coordinate_M) ) {
    ...
    /* External mode */
    {
      ...
      rtExtModePauseIfNeeded(coordinate_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(coordinate_M, true);
      }

      if (rtmGetStopRequested(coordinate_M) == true) {
    ...
    {
      boolean_T rtmStopReq = false;
      rtExtModeOneStep(coordinate_M->extModeInfo, 1, &rtmStopReq);
      if (rtmStopReq) {
        rtmSetStopRequested(coordinate_M, true);
      }
    }
    ...
    {                                  /* Sample time: [0.03s, 0.0s] */
      rtExtModeUpload(0, coordinate_M->Timing.taskTime0);
    }
...
int main(int argc, char **argv)
{
  ...
  fflush(stdout);
  rtmSetErrorStatus(coordinate_M, 0);
  ...
  /* External mode */
  rtSetTFinalForExtMode(&rtmGetTFinal(coordinate_M));
  rtExtModeCheckInit(1);

  {
    boolean_T rtmStopReq = false;
    rtExtModeWaitForStartPkt(coordinate_M->extModeInfo, 1, &rtmStopReq);
    if (rtmStopReq) {
      rtmSetStopRequested(coordinate_M, true);
    }
  }
  ...
  if (rtmGetErrorStatus(coordinate_M) != NULL) {
    printf("\n**%s**\n", rtmGetErrorStatus(coordinate_M));
  }
